...
function acf_course_calculator() {
    ...
    // Check Duration
    $labelDuration = get_field('label-duration','option');
    $unitSingle = get_field('unit-single','option');
    $unitMultiple = get_field('unit-multiple','option');
    $min = get_field('minimale_dauer','option');
    $max = get_field('maximale_dauer','option');
    ...
    if( have_rows('rabatte','option') ): 
        ...
        //echo "<script>var moduleData = $jsonDataModules; var discountData = $jsonDataDiscount;</script>";
    ...
    // other Labels
    $labelLevel = get_field('label-level','option');
    $labelRegPrice = get_field('label-reg-price','option');
    $labelDiscount = get_field('label-dicount','option');
    $labelResult = get_field('label-result','option');
    $labelAverage = get_field('label-average','option');
...
<form id="courseCalculator">
    ...
    <?php if($headline) : ?>
        <div class="col-md-12"><legend><?php echo $headline;?></legend></div>
    <?php endif; ?>
        <div class="col-md-12"><label for="modulSelection"><strong><?php echo ($labelModule) ? $labelModule : '';?></strong></label></div>
    ...
    <?php if( $min ) :  ?>
        <div class="form-group row flex-column duration">
            <div class="col-md-12">
                <label for="kursDauer" class="form-label"><strong><?php echo ($labelDuration) ? $labelDuration : '';?></strong></label>
            </div>
    ...
    <div class="form-group row">
        <div class="col-md-6"><label for="showLevel"><?php echo ($labelLevel) ? $labelLevel : '';?></label></div>
        <div class="col-md-6"><input  class="form-control" id="showLevel" type="text" value="" readonly /></div>
    </div>
    <div class="form-group row">
        <div class="col-md-6"><label for="showPriceReg"><?php echo ($labelRegPrice) ? $labelRegPrice : '';?></label></div>
        <div class="col-md-6"><input class="form-control" id="showPriceReg" type="text" value="" readonly /></div>
    </div>
    <div class="form-group row">
        <div class="col-md-6"><label for="showDiscount"><?php echo ($labelDiscount) ? $labelDiscount : '';?></label></div>
    ...
    <div class="form-group row priceall">
        <div class="col-md-6"><label for="showPriceRegAll"><strong><?php echo ($labelResult) ? $labelResult : '';?></strong></label></div>
        <div class="col-md-6"><input class="form-control" id="showPriceAll" type="text" value="" readonly /></div>
    </div>
    <div class="form-group row">
        <div class="col-md-6"><label for="showPriceRegAll"><?php echo ($labelAverage) ? $labelAverage : '';?></label></div>
        <div class="col-md-6">
